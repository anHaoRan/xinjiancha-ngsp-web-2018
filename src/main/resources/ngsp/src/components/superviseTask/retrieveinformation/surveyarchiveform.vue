      ...
      <el-row style="margin: 20px 0 15px 30px;">
        ...
        <el-col :span="18">
          <el-input :disabled="isabledwrite" placeholder="字符限制200以内" v-bind:maxlength=200 type="textarea"
                    v-model="auditoption" size="medium"
                    style="width:92.5%"></el-input>
        </el-col>
      </el-row>
    ...
    methods: {
      ...
      getAllData () {
        getsurveycheckform(this.instanceId).then((resp) => {
          ...
          this.uploadReturnLetterUrl = '/ngsp/bpm/v1/attachment/upload?fileLoc=obd_returnLetter_attachment&infoId=' + this.infoId + '&fileName=' + this.fileName
          ...
          for (let time of this.accounthistorydata) {
            time.startDate = tabletimeformat(time.startDate)
          }
          for (let time of this.accounthistorydata) {
            time.endDate = tabletimeformat(time.endDate)
          }
          this.getfilelist()
        })
      },
