...
<script>
  ...
  export default {
    ...
    watch: {
      infoId: {
        handler(newValue) {
          this.uploadBasicUrl = uploadFileCommon(newValue, 'obd_letter_send') // infoId, fileLoc
          this.uploadBasicUrl1 = uploadFileCommon(newValue, 'obd_returnLetter_attachment')
        }
      },
      'formCard5.fileList': {
        handler() {
    ...
    data() {
      ...
      return {
        letterTypeOptions,
        msgtargetOptions,
        uploadBasicUrl: uploadFileCommon(this.infoId, 'obd_letter_send'),
        uploadBasicUrl1: uploadFileCommon(this.infoId, 'obd_returnLetter_attachment'),
        rules: {
          letterType: {
