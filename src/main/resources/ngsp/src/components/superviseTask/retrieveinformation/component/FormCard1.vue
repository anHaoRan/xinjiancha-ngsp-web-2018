    ...
    <el-form :model="formCard1" ref="formCard1" label-width="145px" :rules="rules">
      ...
      <el-col :xl="12" :lg="12" :md="12" :sm="24">
        <el-form-item label="异常行为：" prop="abnormalBehav">
          <el-select class="custom-width" :disabled="disabled" v-model="formCard1.abnormalBehav" placeholder="" size="small">
            <el-option
...
<script>
  ...
  import {
    getsurveyaccountinfo, uploadFileCommon
  ...
  export default {
    ...
    watch: {
      ...
      infoId(newValue) {
        this.uploadBasicUrl = uploadFileCommon(newValue, 'obd_BasicInfo_attachment')
      },
      createType(newValue) {
    ...
    data() {
      ...
      return {
        ...
        loading: false,
        addAccountCodeText: '',
        uploadBasicUrl: uploadFileCommon(this.infoId, 'obd_BasicInfo_attachment'),
        abnormalBehavOptions: abnormalBehavOptionsList[14], // 默认调取资料
        tableData: [],
        uploadOption: {
          name: '指定导入',
          size: 'small',
    ...
    methods: {
      ...
      updateAccountInfoByAccounts (TxtData, enterSearchFlag) {
        ...
        this.loading = true
        getsurveyaccountinfo(TxtData.join(',')).then((resp) => {
          this.loading = false
