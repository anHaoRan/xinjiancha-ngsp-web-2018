<template>
  <div class='focusMonitorAccountGroup'>
    <breadcrumb :breadcrumbList='breadcrumbItems'></breadcrumb>
    <div>
      <el-card class='el-card-table'>
        <div slot='header'>
          查询条件
          <el-button type='primary' size='small' @click='newBuilt'>新建监控账户组</el-button>
        </div>
          ...
          <el-row :gutter='20'>
            ...
            <el-col :xl='6' :lg='8' :md='12' :sm='24'>
              <el-form-item label='账户组名称'>
                <el-input v-model='selectParams.groupName' size='small'></el-input>
              </el-form-item>
            </el-col>
            <el-col :xl='6' :lg='8' :md='12' :sm='24'>
...
<script>
  ...
  import {
    postFocusAccountGroupTable,
    deleteAccountGroupStock,
    recallFlow
  ...
  export default {
    ...
    data() {
      return {
        ...
        monitorSecurityColumns: [
          ...
          {label: '账户组名称', field: 'accountName', align: 'center', minWidth: '6%'},
          {label: '账户组代码', field: 'acccountCode', align: 'center', minWidth: '6%'},
    ...
    mounted() {
      ...
      if (loadVuexData) {
        let {groupName, applicant, monitorLevel, localStatus, monitorReason, handler, applyDate} = this.$store.state.superviseTask.focusMonitorAccountGroup
        this.selectParams.groupName = groupName
    ...
    methods: {
      commitQueryParams() {
        let params = {
          groupName: this.selectParams.groupName, // 账户组名称
        ...
        this.$store.commit('saveFocusMonitorSecGroupQueryParams', params)
      ...
      query() {
        ...
        let params = {
          'securityId': !this.selectParams.groupName ? 'ALL' : this.selectParams.groupName,
        ...
        postFocusAccountGroupTable(JSON.stringify(params)).then(resp => {
      ...
      newBuilt() {
        this.$router.push({name: 'newBuiltAccountGroup'})
      },
      jumpPage(row) {
        this.commitQueryParams()
      ...
      yesRecall() {
        if (this.reCallflag) {
          ...
          deleteAccountGroupStock(JSON.stringify(this.deleteparams)).then(resp => {
            if (resp) {
              this.isShow = false
