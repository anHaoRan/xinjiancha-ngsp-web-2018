  ...
  <div>
    ...
    <el-card style="margin-top: 0px;">
      <el-row style="margin: 20px 0 15px 30px;">
        ...
        <el-col :span="10">
          <el-input :disabled="String(handleStatus) === '1'" v-bind:maxlength=50 v-model="name" placeholder="" size="small" style="width:76%"
        ...
        <el-col :span="8">
          <el-input :disabled="String(handleStatus) === '1'" v-bind:maxlength=50 v-model="number" placeholder="" size="small" style="width:95%"
                    @change="isdisplayaction"></el-input>
        </el-col>
      ...
      <el-row style="margin: 20px 0 15px 30px;">
        ...
        <el-col :span="2" v-if="nullunit" class="verfynull" style="margin-left:-9%">来函单位不能为空!</el-col>
      ...
      <el-row style="margin: 20px 0 15px 30px;">
        ...
        <el-col :span="10">
          <el-select :disabled="String(handleStatus) === '1'" filterable v-model="way" placeholder="" size="small" style="width:76%;" @change="changematter">
            <el-option
              v-for="item in isway"
              :key="item.value"
              :label="item.label"
      ...
      <el-row style="margin: 0px 0 15px 30px;">
        ...
        <el-col :span="4">
          <el-select :disabled="String(handleStatus) === '1'" filterable v-model="matter1" placeholder="" size="small" style="width:90%;" @change="changematter">
            <el-option
        ...
        <el-col :span="2" v-if="nullmatter" class="verfynull" style="margin-left:-1%">来函事项不能为空!</el-col>
      </el-row>
      <el-row style="margin: 0px 0 15px 30px;">
      ...
      <el-row style="margin: 20px 0 15px 30px;">
        ...
        <el-col :span="18">
          <el-input :disabled="String(handleStatus) === '1'" placeholder="字符限制50以内" v-bind:maxlength=50 type="textarea" v-model="remarktext" size="medium"
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <a style="text-decoration:underline;color:#409eff;cursor:pointer;"
                 @click="historyrecord(scope.row)">{{scope.row.letterName}}</a>
            </template>
          </el-table-column>
          <el-table-column
            prop="letterDate"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      historyrecord (row) {
        window.open(`#/complish?taskId=${''}&instanceId=${row.infoId}&operate=${'1'}`)
      },
      displayNullStyle () {
      ...
      displaySelectPerson () {
        if ((!this.matter1) && (!matter2)) {
          this.nullmatter = true
          return
        }
        ...
        if (this.commonData.stockCode == '' || this.commonData.stockCode == null || !this.commonData.stockCode.length) {
          this.nullstockcode = true
          return
        }
        this.selectPersonFlag = true
        this.selectData.dialogFormVisble = true
        this.selectData.activityName = '组长分配'
      ...
      saveall () {
        ...
        if (this.commonData.stockCode == '' || this.commonData.stockCode == null) {
          this.nullstockcode = true
          return
        }
      ...
      getAllData () {
        getreplyletterform(this.instanceId).then((resp) => {
          ...
          this.uploadReturnLetterInfoUrl = '/ngsp/bpm/v1/attachment/upload?fileLoc=uns_check_LetterInfo_attachment&infoId=' + this.infoId + '&fileName=' + this.fileName
          if (!resp.unsCheckReceive.extend2) {
            // debugger
            this.createType = '1'
          } else {
            // debugger
      ...
      submitfrom (person) {
        ...
        let isparams = JSON.stringify(params)
        postreplyletterprocessform(isparams).then(() => {
          this.goBack()
        })
      },
      geturlparam () {
