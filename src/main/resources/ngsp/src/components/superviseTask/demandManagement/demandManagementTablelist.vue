<template>
  <div class="demandManagementTable">
    <breadcrumb :breadcrumbList="breadcrumbItems"></breadcrumb>
    <div style="position:absolute;right:114px;top:10px;">
      <el-button @click="routerProcessPage" size="small" type="primary">新建需求管理</el-button>
    </div>
    <el-card>
...
<script>
  ...
  import { demandManagementlist, demandManagementdelete } from '../../../service/superviseTask/index.js'
  ...
  export default {
    ...
    mounted () {
      ...
      if (loadVuexData) {
        let {taskNumber, keyWord, createPeople, localStatus, creatTime} = this.$store.state.superviseTask.demandManagement
    ...
    methods: {
      ...
      commitQueryParams () {
        ...
        this.$store.commit('saveDemandManageQueryParams', params)
      ...
      handledeleteform (row) {
        ...
        if (row.operateDelete === '删除') {
          this.$confirm('确定删除？', '提示', {
            ...
            demandManagementdelete(params).then(() => {
              this.getAllData()
            })
          }).catch(() => {})
        }
      ...
      handleform (row) {
        ...
        this.$router.push({path: `/demandManagementForm?operate=${isoperateCheck}&taskId=${row.taskId}&instanceId=${row.instanceId}&processStatus=${row.localStatusCode}`})
      },
      getAllData () {
        let params = {
        ...
        this.loading = true
        demandManagementlist(JSON.stringify(params)).then((resp) => {
          this.loading = false
