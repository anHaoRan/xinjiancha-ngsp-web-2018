    ...
    <el-card style="margin-top: 0px;">
      ...
      <div v-if="deailflag1">
        <el-row style="margin: 20px 0 15px 30px;" v-if="differentiationflag">
          ...
          <el-col :span="18">
            <el-radio v-model="differentiationadio" label="0">涉嫌庄股操纵</el-radio>
            <el-radio v-model="differentiationadio" label="1" style="margin-left:72px">频繁参与异动股炒作</el-radio>
            <el-radio v-model="differentiationadio" label="2" style="margin-left:30px">维护市场稳定</el-radio>
            <el-radio v-model="differentiationadio" label="3" style="margin-left:55px">其他情节较为严重的异常行为</el-radio>
          </el-col>
        </el-row>
        <el-row style="margin: 20px 0 15px 30px;">
  ...
  export default {
    ...
    mounted () {
      ...
      this.reqActivityNameByProcessName(() => {
        this.geturlparam()
        if (taskId == 'true') {
          let warningNo = instanceId
          getcompeletedata(warningNo).then((resp) => {
            // url无状态时，需要获取接口返回
              ...
              for (let i = 0; i < ismeasureradio.length; i++) {
                switch (ismeasureradio[i]) {
                  ...
                  case '3':
                    ...
                    break
                  case '4':
                    this.measureradio1 = '4'
                    break
                  case '5':
                    this.measureradio2 = '5'
                    break
                }
              }
              if (this.measureradio1 == '4') {
                this.limittable = true
                this.slsRestricttrades = resp.slsRestricttrades
              } else {
                this.limittable = false
              }
              if (this.measureradioa == true || this.measureradiob == true) {
