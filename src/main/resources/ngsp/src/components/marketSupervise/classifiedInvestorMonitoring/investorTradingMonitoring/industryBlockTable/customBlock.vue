...
<script>
  ...
  export default {
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            sortMethod: this.gfnSortColByNumber.bind(this, 'mktqty')
          },
          {
            label: '净成交金额(万)',
            field: 'net_amount',
    ...
    methods: {
      rowClick (row) {
        this.$refs.cusPlateVoc.toggleRowSelection(row)
      },
      ...
      getIndustryCusData () {
        ...
        params.vocType = 'cus' // 审万行业传sw,万德概念传wd,自定义传cus,股票传stock
        getIndustryBlockApi(JSON.stringify(params)).then(resp => {
          this.cusPlateVocData = resp.DATA
        })
      }
    }
  }
