  ...
  <el-card class="el-card-table">
    <div slot="header">
      ...
      <div class="card-operation-box">
        <i class="fa el-icon-download" @click="industriesExcel" style="font-size: 24px"></i>
    ...
    <el-table
      ...
      style="width: 100%;">
      <el-table-column
        v-for="column in industriesColumns"
        :key="column.field"
        :align="column.align"
        :prop="column.field"
        :label="column.label"
    ...
    <div id="hodingIndustriesPieChart" class="chartCls" v-show="!showChart"></div>
  </el-card>
</template>
  ...
  export default {
    ...
    data () {
      return {
        ...
        industriesColumns: [
          {label: '行业（申万行业分类）', field: 'industry_sw1', align: 'center', minWidth: '100', sortable: true},
          {
            label: '持股市值(亿元)',
            field: 'position_val',
            align: 'center',
    ...
    methods: {
      ...
      industriesExcel () {
        ...
        param.gridType = 'PositionCalssify'
        param.tableColumns = this.industriesColumns
      ...
      formatterPieData (data) {
        ...
        for (let idx in data) {
          let item = data[idx]
          legendArr.push(item.industry_sw1)
          dataArr.push({
            value: [item.rate, item.position_val],
            name: item.industry_sw1
          })
        }
        return {
          legendArr,
      ...
      drawPieChart (legendArr, dataArr) {
        ...
        this.pieChart = echarts.init(document.getElementById('hodingIndustriesPieChart'), this.gfnGetTheme())
