<template>
  <div>
    ...
    <div id="stockWeekKChart" class="stock-w-chart"></div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      getStockWeekData () {
        ...
        params.candleStyle = 'W'
        getMarketDayLine(params).then(resp => {
          this.chart = drawStockCandleChart(resp.chartData, 'stockWeekKChart', resp.secCode ? (resp.secCode + ' ' + resp.secName) : '', this.currentType)
