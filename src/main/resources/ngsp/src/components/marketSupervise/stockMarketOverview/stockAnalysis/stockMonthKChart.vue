<template>
  <div>
    ...
    <div id="stockMonthKChart" class="stock-m-chart"></div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      getStockMonthData () {
        ...
        params.candleStyle = 'M'
        getMarketDayLine(params).then(resp => {
          this.chart = drawStockCandleChart(resp.chartData, 'stockMonthKChart', resp.secCode ? (resp.secCode + ' ' + resp.secName) : '', this.currentType)
