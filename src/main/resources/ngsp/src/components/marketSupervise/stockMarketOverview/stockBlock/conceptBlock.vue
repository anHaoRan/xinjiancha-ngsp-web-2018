    ...
    methods: {
      ...
      selectionChange (rows) {
        ...
        if (rows.vocation_code) {
          ...
          this.$refs.conceptBlock.clearSelection()
          this.$refs.conceptBlock.toggleRowSelection(rows, true)
        } else {
          for (let row of rows) {
      ...
      getConceptData () {
        getMonitorStockApi(this.$store.getters.getBoardParams).then(resp => {
          if (resp && resp.bizData) {
            this.conceptBlockData = resp.bizData.metaData
            let boardCode = this.$store.getters.getCurrentBoardCode
            this.$nextTick(function () {
              this.conceptBlockData.forEach(row => {
                if (boardCode.indexOf(row.vocation_code) > -1) {
                  this.$refs.conceptBlock.toggleRowSelection(row, true)
                }
              })
            })
          } else {
            this.conceptBlockData = []
          }
        })
      }
    }
