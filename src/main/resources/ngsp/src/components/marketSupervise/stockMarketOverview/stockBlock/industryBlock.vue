    ...
    methods: {
      ...
      selectionChange (rows) {
        ...
        if (rows.vocation_code) {
          ...
          this.$refs.industryBlock.clearSelection()
          this.$refs.industryBlock.toggleRowSelection(rows, true)
        } else {
          for (let row of rows) {
      ...
      getIndustryData () {
        getMonitorStockApi(this.$store.getters.getBoardParams).then(resp => {
          if (resp && resp.bizData) {
            this.industryBlockData = resp.bizData.metaData
            let boardCode = this.$store.getters.getCurrentBoardCode
            this.$nextTick(function () {
              this.industryBlockData.forEach(row => {
                if (boardCode.indexOf(row.vocation_code) > -1) {
                  this.$refs.industryBlock.toggleRowSelection(row, true)
                }
              })
            })
          } else {
            this.industryBlockData = []
          }
        })
      }
    }
