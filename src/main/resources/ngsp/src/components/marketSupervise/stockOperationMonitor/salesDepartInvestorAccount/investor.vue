  ...
  export default {
    ...
    methods: {
      ...
      getInvData () {
        ...
        params.boardCodeList = this.$store.state.marketSupervise.boardCodeList // 板块代码复数,可为空
        ...
        params.invesType = 'inves' // inves表示分类投资者,cusInves表示自定义分类,account表示账户,branch表示营业部
        params.pageIndex = 1 // 分页处理 表示第几页
        getMonitorInvestorStock(params).then(resp => {
          this.investorData = resp.DATA
        })
      }
    }
  }
