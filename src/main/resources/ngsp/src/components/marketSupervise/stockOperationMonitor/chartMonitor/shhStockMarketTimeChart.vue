<template>
  <div class="shh-stock-market-analysis-chart">
    <div class="chart-content">
      <div id="ShhStockMonitorTime" class="stock-chart"></div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 证券运行监测-股票时分图数据
      getMarketLineData () {
        let param = JSON.parse(JSON.stringify(this.$store.getters.getMarketTimeLineParams))
        getMarketTimeLine(param).then(resp => {
          if (this.chart === null) {
            this.chart = drawStockTimeSharing(resp, 'ShhStockMonitorTime')
            ...
            _this.chart.on('brushSelected', function (val) {
              ...
              if (timeData) {
                // 同步时间选择器的时间
                let st, et
                let timeList = _this.chart.getOption().xAxis[0].data
                ...
                _this.$store.commit(types.M_CHANGE_TIME_RANGE, [startTime, endTime])
              } else {
      ...
      tooltipFormat (params) { // eslint-disable-line
        let fm = []
        let fmx = false
      ...
      getLiquidityMonitorData () {
        if (!this.$store.state.marketSupervise.liquidityMonitor) {
          this.option.legend[0].selected['实时流动性监测'] = false
          this.option.yAxis[3].show = false
          this.chart.setOption(this.option)
          return
        }
        let indexCode = this.$store.state.marketSupervise.currentIndexCode
      ...
      getDeviationRateData () {
        if (!this.$store.state.marketSupervise.deviationRate) {
          this.option.legend[0].selected['大小盘乖离率'] = false
          this.option.yAxis[4].show = false
          this.chart.setOption(this.option)
          return
        }
        let tradeDate = this.$store.state.marketSupervise.tradeDate === '' ? '' : moment(this.$store.state.marketSupervise.tradeDate).format('YYYYMMDD')
      ...
      // 调整时间框,框选联动
      changeLineBrush () {
        let range = [moment(this.$store.state.marketSupervise.startTime).format('HH:mm'), moment(this.$store.state.marketSupervise.endTime).format('HH:mm')]
        let areas = []
...
<style lang="less">
  // 上证指数图
  .shh-stock-market-analysis-chart {
    margin-top: -18px;
