  ...
  export default {
    ...
    watch: {
      codeType: function (type) {
        if (type === 'board-sw') {
          this.$store.commit(types.M_BOARD_CODE_LIST, this.boardSwList)
        }
      }
    },
    methods: {
      selectionChange (rows) {
        ...
        if (rows.vocation_code) {
          ...
          this.$refs.swVoc.clearSelection()
          this.$refs.swVoc.toggleRowSelection(rows, true)
        } else {
          for (let row of rows) {
            codes += row.vocation_code + ','
          }
        }
        this.boardSwList = codes.substring(0, codes.length - 1)
        this.$store.commit(types.M_BOARD_CODE_LIST, this.boardSwList)
      ...
      getSwData () {
        getMonitorStockApi(this.$store.getters.getMonitorBlockParams).then(resp => {
          if (resp && resp.bizData) {
            this.swVocData = resp.bizData.metaData
            let boardCodeList = this.$store.getters.getBoardCodeList
            this.$nextTick(function () {
              this.swVocData.forEach(row => {
                if (boardCodeList.indexOf(row.vocation_code) > -1) {
                  this.$refs.swVoc.toggleRowSelection(row, true)
                }
              })
            })
          }
