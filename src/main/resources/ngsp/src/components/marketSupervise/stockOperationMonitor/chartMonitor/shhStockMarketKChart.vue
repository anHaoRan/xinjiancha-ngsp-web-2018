<template>
  <div class="shh-stock-market-analysis-chart">
    <div class="chart-content">
      ...
      <div id="stockOperateMonDayK" class="stock-chart"></div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      changeCandleChartBrush () {
        let startDate = this.$store.state.marketSupervise.startDate
        let endDate = this.$store.state.marketSupervise.endDate
        let range = null
      ...
      getMarketKData () {
        ...
        getMarketDayLine(params).then(resp => {
          ...
          _this.chart = drawStockCandleChart(resp.chartData, 'stockOperateMonDayK')
          _this.chart.on('click', function (val) {
            _this.$store.commit(types.M_CHANGE_TRADE_DATE, val.name)
          })
          ...
          _this.chart.on('brushSelected', function (val) {
            if (!val.batch[0].areas.length) {
              ...
              _this.$store.commit(types.M_IS_BRUSH, false)
              _this.$store.commit(types.M_CHANGE_DATE_RANGE, ['', ''])
              return
            }
            let dateList = _this.chart.getOption().xAxis[0].data
            let rangeIdxs = val.batch[0].areas[0].coordRange
            _this.$store.commit(types.M_IS_BRUSH, true)
            ...
            _this.$store.commit(types.M_CHANGE_DATE_RANGE, [new Date(dateList[startIdx]), new Date(dateList[endIdx])])
