  ...
  export default {
    ...
    watch: {
      codeType: function (type) {
        if (type === 'board-cus') {
          this.$store.commit(types.M_BOARD_CODE_LIST, this.boardCusList)
        }
      }
    },
    methods: {
      selectionChange (rows) {
        ...
        if (rows.vocation_code) {
          ...
          this.$refs.cusPlateVoc.clearSelection()
          this.$refs.cusPlateVoc.toggleRowSelection(rows, true)
        } else {
          for (let row of rows) {
            codes += row.vocation_code + ','
          }
        }
        this.boardCusList = codes.substring(0, codes.length - 1)
        this.$store.commit(types.M_BOARD_CODE_LIST, this.boardCusList)
      ...
      getCusData () {
        getMonitorStockApi(this.$store.getters.getMonitorBlockParams).then(resp => {
          if (resp && resp.bizData) {
            this.cusVocData = resp.bizData.metaData
            let boardCodeList = this.$store.getters.getBoardCodeList
            this.$nextTick(function () {
              this.cusVocData.forEach(row => {
                if (boardCodeList.indexOf(row.vocation_code) > -1) {
                  this.$refs.cusPlateVoc.toggleRowSelection(row, true)
                }
              })
            })
          }
