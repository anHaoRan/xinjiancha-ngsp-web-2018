    ...
    <el-card class='el-card-table'>
      <div style='display: flex'>
        <el-tabs v-model='activeName' type='card' tab-position='left' @tab-click='changeTabs'>
          <el-tab-pane label='板块' name='first' :disabled='tableTwoStatus === "板块"'>
          </el-tab-pane>
          <el-tab-pane label='股票' name='second' :disabled='tableTwoStatus === "股票"'>
          </el-tab-pane>
          <el-tab-pane label='公司' name='three' :disabled='tableTwoStatus === "公司"'>
          </el-tab-pane>
          <el-tab-pane label='交易会员' name='four' :disabled='tableTwoStatus === "交易会员"'>
          </el-tab-pane>
          <el-tab-pane label='交易营业部' name='five' :disabled='tableTwoStatus === "交易营业部"'>
          </el-tab-pane>
          <el-tab-pane label='分类投资者' name='six' :disabled='tableTwoStatus === "分类投资者"'>
          </el-tab-pane>
          <el-tab-pane label='自定义投资者' name='seven' :disabled='tableTwoStatus === "自定义投资者"'>
          </el-tab-pane>
          <el-tab-pane label='账户' name='eight' :disabled='tableTwoStatus === "账户"'>
          </el-tab-pane>
        </el-tabs>
        <el-table
          v-if='isShow'
...
<script>
  ...
  export default {
    ...
    watch: {
      tableTwoStatus: function (param1, param2) {
        if (param1 != param2) {
          this.activeName = ''
          this.tableData = []
          this.tableColumns = []
    ...
    computed: {
      tableTwoStatus() {
        return this.$store.state.highQuery.tableTwo.tabName
      },
      brushTimeParams() {
    ...
    methods: {
      ...
      selectedRow(row) {
        ...
        this.$store.commit('savetableThreeRows', params.join(','))
      },
      // 表格行选中
      selectedAllRow(row) {
        let params = []
      ...
      getTableData(tabName) {
        let paramsCode = this.$store.state.highQuery.tableTwoRows
        ...
        let dimParams = {
          ...
          'topN': 100,
          'indusCodes': this.tableTwoStatus == '板块' ? paramsCode : '',
          'securityIds': this.tableTwoStatus == '股票' ? paramsCode : '',
          'memberIds': this.tableTwoStatus == '交易会员' ? paramsCode : '',
          'compCodes': this.tableTwoStatus == '公司' ? paramsCode : '',
          'brnIds': this.tableTwoStatus == '交易营业部' ? paramsCode : '',
          'investorTypecodes': this.tableTwoStatus == '分类投资者' ? paramsCode : '',
          'custorgCodes': this.tableTwoStatus == '自定义投资者' ? paramsCode : '',
          'acctIds': this.tableTwoStatus == '账户' ? paramsCode : ''
        }
        let indicParams = {
          'tradeDateBegin': this.startTime,
          'tradeDateEnd': this.endTime,
        ...
        if (tabsName === '板块') {
          if (this.tabsNameParams == 'sharingTime') {
            if (this.tableTwoStatus === '板块' || this.tableTwoStatus == '股票') {
              params = {
                'dim': {
        ...
        if (tabName != this.lastLable) {
          ...
          this.$store.commit('savetableThreeParams', {status: true, tabName: tabName})
