    ...
    <el-card class='el-card-table'>
      <div style='display: flex'>
        <el-tabs v-model='activeName' type='card' tab-position='left' @tab-click='changeTabs'>
          <el-tab-pane label='板块' name='first' :disabled='tableFiveStatus === "板块"'>
          </el-tab-pane>
          <el-tab-pane label='股票' name='second' :disabled='tableFiveStatus === "股票"'>
          </el-tab-pane>
          <el-tab-pane label='公司' name='three' :disabled='tableFiveStatus === "公司"'>
          </el-tab-pane>
          <el-tab-pane label='交易会员' name='four' :disabled='tableFiveStatus === "交易会员"'>
          </el-tab-pane>
          <el-tab-pane label='交易营业部' name='five' :disabled='tableFiveStatus === "交易营业部"'>
          </el-tab-pane>
          <el-tab-pane label='分类投资者' name='six' :disabled='tableFiveStatus === "分类投资者"'>
          </el-tab-pane>
          <el-tab-pane label='自定义投资者' name='seven' :disabled='tableFiveStatus === "自定义投资者"'>
          </el-tab-pane>
          <el-tab-pane label='账户' name='eight' :disabled='tableFiveStatus === "账户"'>
          </el-tab-pane>
        </el-tabs>
        <el-table
          v-if='isShow'
...
<script>
  ...
  export default {
    ...
    watch: {
      tableFiveStatus: function (param1, param2) {
        if (param1 != param2) {
          this.activeName = ''
          this.tableData = []
          this.tableColumns = []
    ...
    computed: {
      tableFiveStatus() {
        return this.$store.state.highQuery.tableFive.tabName
      },
      brushTimeParams() {
    ...
    methods: {
      ...
      getTableData(tabName) {
        let paramsCode = this.$store.state.highQuery.tableFiveRows
        ...
        let dimParams = {
          ...
          'topN': 100,
          'indusCodes': this.tableFiveStatus == '板块' ? paramsCode : '',
          'securityIds': this.tableFiveStatus == '股票' ? paramsCode : '',
          'memberIds': this.tableFiveStatus == '交易会员' ? paramsCode : '',
          'compCodes': this.tableFiveStatus == '公司' ? paramsCode : '',
          'brnIds': this.tableFiveStatus == '交易营业部' ? paramsCode : '',
          'investorTypecodes': this.tableFiveStatus == '分类投资者' ? paramsCode : '',
          'custorgCodes': this.tableFiveStatus == '自定义投资者' ? paramsCode : '',
          'acctIds': this.tableFiveStatus == '账户' ? paramsCode : ''
        }
        let indicParams = {
          'tradeDateBegin': this.startTime,
          'tradeDateEnd': this.endTime,
        ...
        if (tabsName === '板块') {
          if (this.tabsNameParams == 'sharingTime') {
            if (this.tableFiveStatus === '板块' || this.tableFiveStatus == '股票') {
              params = {
                'dim': {
        ...
        this.$store.commit('savetableSixParams', {status: true, tabName: tabName})
      },
      changeTabs(val) {
        if (val.label == '账户') {
          if (this.flag) {
