    ...
    <el-card class='el-card-table'>
      <div style='display: flex'>
        <el-tabs v-model='activeName' type='card' tab-position='left' @tab-click='changeTabs'>
          <el-tab-pane label='板块' name='first' :disabled='tableOneStatus === "板块"'>
          </el-tab-pane>
          <el-tab-pane label='股票' name='second' :disabled='tableOneStatus === "股票"'>
          </el-tab-pane>
          <el-tab-pane label='公司' name='three' :disabled='tableOneStatus === "公司"'>
          </el-tab-pane>
          <el-tab-pane label='交易会员' name='four' :disabled='tableOneStatus === "交易会员"'>
          </el-tab-pane>
          <el-tab-pane label='交易营业部' name='five' :disabled='tableOneStatus === "交易营业部"'>
          </el-tab-pane>
          <el-tab-pane label='分类投资者' name='six' :disabled='tableOneStatus === "分类投资者"'>
          </el-tab-pane>
          <el-tab-pane label='自定义投资者' name='seven' :disabled='tableOneStatus === "自定义投资者"'>
          </el-tab-pane>
          <el-tab-pane label='账户' name='eight' :disabled='tableOneStatus === "账户"'>
          </el-tab-pane>
        </el-tabs>
        <el-table
          v-if='isShow'
...
<script>
  ...
  export default {
    ...
    watch: {
      tableOneStatus: function (param1, param2) {
        if (param1 != param2) {
          this.activeName = ''
          this.tableData = []
          this.tableColumns = []
      ...
      brushTimeParams: {
        handler: function (param1, param2) {
          this.startTime = moment(param1.startTime, 'YYYY/MM/DD').format('YYYYMMDD')
          this.endTime = moment(param1.overTime, 'YYYY/MM/DD').format('YYYYMMDD')
        },
        deep: true
      }
    },
    computed: {
      tableOneStatus() {
        return this.$store.state.highQuery.tableOne.tabName
      },
      brushTimeParams() {
    ...
    methods: {
      ...
      selectedRow(row) {
        ...
        this.$store.commit('savetableTwoRows', params.join(','))
      },
      // 表格行选中
      selectedAllRow(row) {
        let params = []
      ...
      getTableData(tabName) {
        let paramsCode = this.$store.state.highQuery.tableOneRows
        ...
        let dimParams = {
          ...
          'topN': 100,
          'indusCodes': this.tableOneStatus == '板块' ? paramsCode : '',
          'securityIds': this.tableOneStatus == '股票' ? paramsCode : '',
          'memberIds': this.tableOneStatus == '交易会员' ? paramsCode : '',
          'compCodes': this.tableOneStatus == '公司' ? paramsCode : '',
          'brnIds': this.tableOneStatus == '交易营业部' ? paramsCode : '',
          'investorTypecodes': this.tableOneStatus == '分类投资者' ? paramsCode : '',
          'custorgCodes': this.tableOneStatus == '自定义投资者' ? paramsCode : '',
          'acctIds': this.tableOneStatus == '账户' ? paramsCode : ''
        }
        let indicParams = {
          'tradeDateBegin': this.startTime,
          'tradeDateEnd': this.endTime,
        ...
        if (tabsName === '板块') {
          if (this.tabsNameParams == 'sharingTime') {
            if (this.tableOneStatus === '板块' || this.tableOneStatus === '股票') {
              params = {
                'dim': {
        ...
        if (tabName != this.lastLable) {
          ...
          this.$store.commit('savetableTwoParams', {status: true, tabName: tabName})
