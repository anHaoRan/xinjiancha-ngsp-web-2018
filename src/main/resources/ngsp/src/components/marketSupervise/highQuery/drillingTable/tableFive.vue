    ...
    <el-card class='el-card-table'>
      <div style='display: flex'>
        <el-tabs v-model='activeName' type='card' tab-position='left' @tab-click='changeTabs'>
          <el-tab-pane label='板块' name='first' :disabled='tableFourStatus === "板块"'>
          </el-tab-pane>
          <el-tab-pane label='股票' name='second' :disabled='tableFourStatus === "股票"'>
          </el-tab-pane>
          <el-tab-pane label='公司' name='three' :disabled='tableFourStatus === "公司"'>
          </el-tab-pane>
          <el-tab-pane label='交易会员' name='four' :disabled='tableFourStatus === "交易会员"'>
          </el-tab-pane>
          <el-tab-pane label='交易营业部' name='five' :disabled='tableFourStatus === "交易营业部"'>
          </el-tab-pane>
          <el-tab-pane label='分类投资者' name='six' :disabled='tableFourStatus === "分类投资者"'>
          </el-tab-pane>
          <el-tab-pane label='自定义投资者' name='seven' :disabled='tableFourStatus === "自定义投资者"'>
          </el-tab-pane>
          <el-tab-pane label='账户' name='eight' :disabled='tableFourStatus === "账户"'>
          </el-tab-pane>
        </el-tabs>
        <el-table
          v-if='isShow'
...
<script>
  ...
  export default {
    ...
    watch: {
      tableFourStatus: function (param1, param2) {
        if (param1 != param2) {
          this.activeName = ''
          this.tableData = []
          this.tableColumns = []
    ...
    computed: {
      tableFourStatus() {
        return this.$store.state.highQuery.tableFour.tabName
      },
      brushTimeParams() {
    ...
    methods: {
      ...
      selectedRow(row) {
        ...
        this.$store.commit('savetableFiveRows', params.join(','))
      },
      // 表格行选中
      selectedAllRow(row) {
        let params = []
      ...
      getTableData(tabName) {
        let paramsCode = this.$store.state.highQuery.tableFourRows
        ...
        let dimParams = {
          ...
          'topN': 100,
          'indusCodes': this.tableFourStatus == '板块' ? paramsCode : '',
          'securityIds': this.tableFourStatus == '股票' ? paramsCode : '',
          'memberIds': this.tableFourStatus == '交易会员' ? paramsCode : '',
          'compCodes': this.tableFourStatus == '公司' ? paramsCode : '',
          'brnIds': this.tableFourStatus == '交易营业部' ? paramsCode : '',
          'investorTypecodes': this.tableFourStatus == '分类投资者' ? paramsCode : '',
          'custorgCodes': this.tableFourStatus == '自定义投资者' ? paramsCode : '',
          'acctIds': this.tableFourStatus == '账户' ? paramsCode : ''
        }
        let indicParams = {
          'tradeDateBegin': this.startTime,
          'tradeDateEnd': this.endTime,
        ...
        if (tabsName === '板块') {
          if (this.tabsNameParams == 'sharingTime') {
            if (this.tableFourStatus === '板块' || this.tableFourStatus == '股票') {
              params = {
                'dim': {
        ...
        this.$store.commit('savetableFiveParams', {status: true, tabName: tabName})
      },
      changeTabs(val) {
        if (val.label == '账户') {
          if (this.flag) {
