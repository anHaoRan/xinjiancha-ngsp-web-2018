<template>
    <div class="output-result-set">
        <s-table ref="marginTable" headerTitle="标的券交易查询母表" :columns="columns[1]" :data="totalData[1]"
        ...
        <s-table ref="marginTable2" headerTitle="标的券交易查询子表" :columns="columns[2]" :data="totalData[2]"
...
<script>
  import StandardTicketRadio from './StandardTicketRadio'
  import ExportMixin from '../ExportMixin'
  import { getBd, getBdDetail } from '@/service/analysisTool/MarginTrading/Index'
  ...
  export default {
    ...
    computed: {
      getStandardTicketRadio () {
        return this.$store.getters.getStandardTicketRadio
      },
      getStandardTicketDetail () {
        return this.$store.getters.getStandardTicketDetail
      }
    },
    watch: {
      getStandardTicketRadio () {
        if (this.getStandardTicketRadio) {
          this.isShow = true
          this.$nextTick(function () {
            this.ajaxPostBdDetail()
          })
        }
      },
      selectParams: {
    ...
    methods: {
      ajaxPostBd () {
        let {isinType, indexCode, dateType, selectTradeDate} = this.selectParams,
      ...
      ajaxPostBdDetail () {
        ...
        params = {
          ...
          timeStart: '',
          timeEnd: '',
          type: this.getStandardTicketDetail.type,
          statType: this.getStandardTicketDetail.statType,
          isinCode: this.getStandardTicketRadio
        }
        if (dateType === '1') {
          params.timeStart = timeStart
          params.timeEnd = timeEnd
      ...
      handleDownload (val) {
        ...
        params.fileMark = val
        params.markType = 'bdQuery'
        params.tableColumns = [
          {
            sheetName: '融资融券-标的券交易查询母表',
      ...
      handleDownload2 (val) {
        ...
        params.fileMark = val
        params.markType = 'bdQuerySub'
        params.tableColumns = [
          {
            sheetName: '融资融券-标的券交易查询子表',
