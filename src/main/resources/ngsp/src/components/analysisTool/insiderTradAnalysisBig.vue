  ...
  export default {
    ...
    methods: {
      getAnalyPerson (code, name) {
        this.analystCode = name
        this.analystName = code
      },
      ...
      add () {
        this.saveParams = {
          //  taskCode: this.newTaskCon.taskNum,
          stockCode: this.newTaskCon.stockCode,
          //  analUser: this.newTaskCon.addAnalyst,
          //  reAnalUser: this.newTaskCon.addReviewer,
      ...
      analysisStart (scope) {
        ...
        this.$router.push({name: 'insiderTradAnalysisParamsBig', params: {stockCode: scope.row.sec_code}})
      },
      // 表格操作之删除
      deleteTaskCon (scope) {
      ...
      checkEnd (scope) {
        ...
        this.$router.push({name: 'insiderTradAccountAnalysisBig'})
      },
      // 表格操作之查看已技术任务
      checkFinish (scope) {
        let currentTaskData = {
          ...
          operatorType: 'finish',
          taskNum: scope.row.source_pro_id,
          operateType: scope.row.opt
        }
        this.$store.commit('saveCurrentTaskData', currentTaskData)
        let opt = scope.row.opt
        if (opt === '6' || opt === '4') {
          this.$router.push({name: 'insiderTradAnalysisParamsBig', params: {stockCode: scope.row.sec_code}})
        }
      }
    ...
    mounted () {
      if ($.isEmptyObject(this.$store.state.insiderTradAnalysis.taskListData)) {
        ...
        this.analystCode = this.$store.state.insiderTradAnalysis.requestParamsOfTaskList.analystCode
        this.analystName = this.$store.state.insiderTradAnalysis.requestParamsOfTaskList.analystName
        this.reviewerCode = this.$store.state.insiderTradAnalysis.requestParamsOfTaskList.reviewerCode
        this.reviewerName = this.$store.state.insiderTradAnalysis.requestParamsOfTaskList.reviewerName
