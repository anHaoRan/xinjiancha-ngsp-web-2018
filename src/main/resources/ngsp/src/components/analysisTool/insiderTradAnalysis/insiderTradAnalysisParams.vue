          ...
          <el-col :span="15" style="padding: 0">
            <el-card style="height: 600px;margin-right: 15px;">
              ...
              <div style="min-width: 600px">
                ...
                <div v-if="showDrawPieChartTips"
                     style="width: 300px;margin-left: -5px; height: 500px;float: left;padding-top:240px;">
                  在地图上点击省市，可查看该省市内幕知情人分布地级市前5名
                </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getMaxAndMinVal (data) {
        this.mapValData = data.map(item => {
          return item.value
        })
        this.maxVal = Math.max(...this.mapValData)
        this.minVal = Math.min(...this.mapValData)
      },
      // 设置触发函数
      settings () {
        let isEmptyObj = {
      ...
      drawMap (data) {
        ...
        mychart.setOption(option = {
          ...
          visualMap: {
            min: this.minVal - 5 < 0 ? 0 : this.minVal - 5,
            max: this.maxVal + 5,
            left: 'left',
            top: 'bottom',
            selectMode: 'single',
      ...
      drawPieChart (pieList) {
        this.showDrawPieChartTips = false
        ...
        getRelativeBaseDate(0, checkDateStart).then(resp => {
          ...
          getRelativeBaseDate(0, checkDateEnd).then(resp => {
            ...
            getDefaultResumptionDate(params).then(resp => {
              if (resp.resumptionDate && this.resumptionDate === '') {
