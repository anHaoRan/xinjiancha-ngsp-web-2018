...
import { latestMarketPrice, exportLatestMarketPrice } from '@/service/analysisTool/complexQuery/accountBehaviorTracking'
...
export default {
  ...
  computed: {
    ...mapGetters(['queryTrackForm1'])
  },
  watch: {
    'queryTrackForm1': {
      handler() {
        this.queryData = this.queryTrackForm1
        this.reqTableData(1)
      },
      deep: true
    }
  ...
  methods: {
    ...
    reqTableData(type) {
      ...
      this.loading = true
      latestMarketPrice(params).then(resp => {
        this.loading = false
        if (resp) {
    ...
    handleExportData(type) {
      ...
      exportLatestMarketPrice(params)
    }
  }
}
