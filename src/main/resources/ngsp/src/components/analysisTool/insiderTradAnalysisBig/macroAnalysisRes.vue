<template>
  <div class="macro-analysis-res">
    ...
    <div>
      ...
      <div>
        <transaction-stream v-show="'transactionStream' ==  momentPath" ref="transactionStream"></transaction-stream>
        <basic-info v-show="'basicInfo'==  momentPath" ref="basicInfo"></basic-info>
        <hold-degree v-show="'holdDegree'==  momentPath" ref="holdDegree"></hold-degree>
        <single-account-change v-show="'singleAccountChange'==  momentPath"
                               ref="singleAccountChange"></single-account-change>
        <stockholder-history-change v-show="'stockholderHistoryChange'==  momentPath"
                                    ref="stockholderHistoryChange"></stockholder-history-change>
        <newspaper-and-public-opinion v-show="'newspaperAndPublicOpinion'==  momentPath"
                                      ref="newspaperAndPublicOpinion"></newspaper-and-public-opinion>
        <history-regulatory v-show="'historyRegulatory'==  momentPath" ref="historyRegulatory"></history-regulatory>
        <block-trade-info v-show="'blockTradeInfo'==  momentPath" ref="blockTradeInfo"></block-trade-info>
        <list-of-top-hundred v-show="'listOfTopHundred'==  momentPath" ref="listOfTopHundred"></list-of-top-hundred>
        <insider-detail-info v-show="'insiderDetailInfo'==  momentPath" ref="insiderDetailInfo"></insider-detail-info>
      </div>
      <div style="margin: 10px auto ;text-align: center">
        <el-button size="small" type="text" @click="downloadTabsDialog = true" v-if="false">内容导出</el-button>
        <el-button v-if="operateType!=='6'" size="small" type="primary" @click="finishAnaly">完成宏观分析</el-button>
        <el-button v-if="operatorType=='analysiser'&&operateType!=='6'" size="small" type="text"
                   @click="stopTaskDialog = true">终止分析
        </el-button>
        <el-button size="small" type="text" @click="goTaskListPage" v-if="operateType==='6'">返回任务列表</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      handleClick (tab, envent) { // eslint-disable-line
      ...
      finishAnaly () {
        ...
        switch (this.operatorType) {
          ...
          case 'checker':
            ...
            break
          case 'finish':
            this.$router.push({path: '/insiderTradAccountAnalysisBig'})
            break
        }
      },
      // 提交完成任务
        ...
        if (taskStatus === '2' || taskStatus === '4') {
          changeTaskstatus(params).then(resp => {
            if (resp) {
              ...
              this.$router.push({path: '/insiderTradAnalysisBig'})
            } else {
