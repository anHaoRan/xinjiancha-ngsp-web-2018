<template>
    <div class="institutions">
        <div style="display: flex;">
            ...
            <div class="accountDetailTable" style="height: 350px;width: 30%;">
        ...
        <el-card>
            ...
            <el-table
                ...
                <s-table-columns v-for="(item,index) in STABILITY_LIST_ACCOUNT_GRID_YMT"
                                 :key="index"
                                 :item="item"
                                 v-if="item.isActive===undefined||item.isActive"/>
            </el-table>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        STABILITY_LIST_ACCOUNT_GRID_YMT: [
          {
    ...
    methods: {
      ...
      companyBasicInfo () {
        ...
        accountListByYmtno(param).then((res) => {
          ...
          for (var i = 0; i < res.length; i++) {
            str += `${res[i].acctno ? res[i].acctno : ''}   ${res[i].acctname ? res[i].acctname : ''}   ${res[i].branchname ? res[i].branchname : ''} </br>`
          }
          this.copyData = str
        })
      },
      ajaxTrdDetailChart () {
        ...
        trdDetailChartHalfHour(param).then((res) => {
          let arr = res
