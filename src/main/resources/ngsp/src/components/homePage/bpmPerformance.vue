<template>
  <el-card class="small-card">
    <div class="small-card-title">每日流程耗时</div>
    <div class="small-card-content">
      <div id="bpmPerformance" :style="{
          'height':newHeight+'px'
      }"></div>
    </div>
  </el-card>
      ...
      height: {
        handler () {
          this.$nextTick(function () {
            this.initEcharts('bpmPerformance')
    ...
    methods: {
      ajaxGetAlarmTrend () {
        getAlarmTrend().then(res => {
          ...
          this.options.title.text = `{title|${map.td}}{text|min}\n{text|今日平均耗时}\n\n{title|${map.avd}}{text|min}\n{text|总平均耗时}`
          let obj = {
            xdata: [],
            series: []
          }
